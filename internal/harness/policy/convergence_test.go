...
func TestBuildPRChains_singleChain(t *testing.T) {
	...
	// when
	report := policy.BuildPRConvergenceReport("main", []domain.PRState{pr1, pr2, pr3})
...
func TestBuildPRChains_orphanedPR(t *testing.T) {
	...
	// when
	report := policy.BuildPRConvergenceReport("main", []domain.PRState{pr1, pr2})
...
func TestBuildPRChains_emptyPRs(t *testing.T) {
	// when
	report := policy.BuildPRConvergenceReport("main", nil)
...
func TestClassifyConvergenceScenario(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// when
			severity, action := policy.ClassifyConvergenceScenario(tt.chain)
...
func TestBuildPRChains_longerCycle_terminates(t *testing.T) {
	...
	go func() {
		defer close(done)
		report := policy.BuildPRConvergenceReport("main", []domain.PRState{pr1, pr2, pr3, pr4, pr5})
		if len(report.Chains) != 1 {
			t.Errorf("expected 1 chain, got %d", len(report.Chains))
			return
		}
