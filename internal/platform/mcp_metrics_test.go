...
func TestRecordMCPInvocation_IncrementsCounterAndHistogram(t *testing.T) {
	...
	platform.RecordMCPInvocation(ctx, "next_review", "deprecated", 12*time.Millisecond)
...
func TestRecordMCPInvocation_AttributesIncludeToolNameAndStatus(t *testing.T) {
	...
	// when
	platform.RecordMCPInvocation(ctx, "post_comment", "error", 2*time.Millisecond)
		...
		for _, m := range sm.Metrics {
			...
			for _, dp := range sum.DataPoints {
				for _, attr := range dp.Attributes.ToSlice() {
					if string(attr.Key) == "tool.name" && attr.Value.AsString() == "post_comment" {
						foundTool = true
					}
	...
	if !foundTool {
		t.Error("expected tool.name=post_comment attribute on metric data point")
	}
	if !foundStatus {
