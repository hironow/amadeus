...
func NewSQLiteOutboxStore(dbPath, archiveDir, outboxDir string) (*SQLiteOutboxStore, error) { // nosemgrep: domain-primitives.multiple-string-params-go — dbPath/archiveDir/outboxDir are distinct path roles for the transactional outbox [permanent]
...
func atomicWrite(targetPath string, data []byte) error {
	...
	tmp, err := os.CreateTemp(dir, ".amadeus-tmp-*")
	if err != nil {
		return err
	}
	tmpPath := tmp.Name()
