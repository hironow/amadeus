...
func TestStatus_ProviderMetadata(t *testing.T) {
	...
	store, err := session.NewSQLiteCodingSessionStore(filepath.Join(baseDir, ".run", "sessions.db"))
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
	...
	record.Metadata = domain.ProviderStateSnapshot{
		...
		RetryBudget:     0,
		ResumeAt:        time.Date(2026, 4, 5, 16, 40, 0, 0, time.UTC),
	...
	if report.ProviderState != string(domain.ProviderStatePaused) {
		t.Fatalf("ProviderState = %q, want %q", report.ProviderState, domain.ProviderStatePaused)
	}
