...
func TestE2E_Run_DryRun(t *testing.T) {
	...
	stdout, _, err := runCmd(t, dir, "run", "--dry-run")
	if err != nil {
...
func TestE2E_Run_FullCalibration(t *testing.T) {
	dir := initTestRepo(t)
	writeConfig(t, dir, defaultTestConfig())

	// Force full calibration — fake-claude returns fullCalibrationResponse (with D-Mails)
	stdout, stderr, err := runCmd(t, dir, "run", "--full", "--json")
...
func TestE2E_Run_FullCalibration_TextOutput(t *testing.T) {
	dir := initTestRepo(t)
	writeConfig(t, dir, defaultTestConfig())

	stdout, _, err := runCmd(t, dir, "run", "--full")
	assertExitCode(t, err, 2)
...
func TestE2E_Run_FullCalibration_QuietOutput(t *testing.T) {
	dir := initTestRepo(t)
	writeConfig(t, dir, defaultTestConfig())

	stdout, _, err := runCmd(t, dir, "run", "--full", "--quiet")
	assertExitCode(t, err, 2)
...
func TestE2E_Run_DiffCheck_NoDrift(t *testing.T) {
	dir := initTestRepo(t)
	writeConfig(t, dir, defaultTestConfig())
	...
	stdout, _, err := runCmd(t, dir, "run", "--json")
	if err != nil {
...
func TestE2E_Run_SeverityRouting_High(t *testing.T) {
	dir := initTestRepo(t)
...
func TestE2E_Run_SeverityRouting_Low(t *testing.T) {
	dir := initTestRepo(t)
...
func TestE2E_Run_DMailIssuesField(t *testing.T) {
	dir := initTestRepo(t)
	writeConfig(t, dir, defaultTestConfig())
