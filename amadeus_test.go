...
func TestDriftError_IsDrift(t *testing.T) {
	// given: a DriftError
	err := &amadeus.DriftError{Divergence: 0.35, DMails: 2}

	// then: it implements error and is detectable via errors.As
	var de *amadeus.DriftError
	if !errors.As(err, &de) {
...
func TestDriftError_Message(t *testing.T) {
	err := &amadeus.DriftError{Divergence: 0.35, DMails: 1}
	msg := err.Error()
...
func TestExitCode_Nil(t *testing.T) {
	if code := amadeus.ExitCode(nil); code != 0 {
...
func TestExitCode_DriftError(t *testing.T) {
	err := &amadeus.DriftError{Divergence: 0.35, DMails: 2}
	if code := amadeus.ExitCode(err); code != 2 {
...
func TestExitCode_RegularError(t *testing.T) {
	err := fmt.Errorf("something broke")
	if code := amadeus.ExitCode(err); code != 1 {
...
func TestExitCode_WrappedDriftError(t *testing.T) {
	...
	inner := &amadeus.DriftError{Divergence: 0.50, DMails: 3}
